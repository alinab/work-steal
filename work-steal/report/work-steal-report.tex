...
\begin{itemize}
    \item The parallel Fibonacci benchmark reveals that task granularity has a
     ...
     \item At fine granularity (threshold=20), the situation reverses: work-stealing
         reaches 4 times speedup while naive drops to under 4 because the high task count
         causes heavy mutex contention on every push and pop. The steal ratio plots confirm
         this — at threshold=30 up to 26% of tasks are stolen, reflecting genuine
         load imbalance as workers exhaust their local queues quickly,
         while at threshold=20 stealing stays below 2%
         since each worker has abundant local work.
...
The takeaways from our benchmarking are:
\begin{itemize}
    \item Work-stealing peaks at 4 workers regardless of threshold — beyond
    this all configurations degrade as cores compete for the same memory bus,
    confirming that the bottleneck is memory throughput rather than task distribution.
    The naive scheduler shows more dramatic threshold sensitivity: at threshold=2000 and
    threshold=5000 with 2 and 4 workers, several runs deadlocked or livelocked
    (average times exceeding 5 seconds, clipped from the plot),
    indicating that coarser tasks combined with low worker counts
    creates a situation where workers block waiting for tasks that are never released from the queue.
    \item At 8 and 16 workers both schedulers converge to similar speedups
     with naive performing slightly worse due to mutex contention on top of memory
     bus pressure. The steal ratio plots show that mergesort generates higher
     steal ratios than Fibonacci at the same thresholds — up to 14\% at threshold=5000
     with 16 workers — reflecting the structural imbalance of mergesort where
     top-level merge tasks take orders of magnitude longer than leaf tasks,
     forcing idle workers to steal frequently
    \item The throughput plots reinforce this: fine-grained tasks (threshold=500)
     achieve the highest task rate for work-stealing, peaking at around 47,000
     tasks per second at 4 workers, while naive throughput at the same granularity
     is lower and drops sharply beyond 4 workers as mutex contention accumulates.
\end{itemize}
...
materialises in practice. Chase and Lev prove that their algorithm is correct
under the assumption that the owner is the only domain operating on the bottom
end of the deque — our QCheck-Lin and QCheck-STM tests formally verified this
...
almost double speedup at 4 workers and avoids the mutex contention that causes
the naive scheduler to deadlock under certain threshold and worker combinations
— precisely the irregular workload scenario the paper's dynamic array design
...
required a tag field in the earlier ABP algorithm of \citet{arora1998thread}.
