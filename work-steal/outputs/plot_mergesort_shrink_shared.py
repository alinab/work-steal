...
def fig_fixed_workers(idx, shrink_policies, pool_modes, sizes, size_ranks,
    ...
    metrics = [
        ("speedup",     "speedup (x)",       f"speedup  (workers={workers_max})"),
        ("avg",         "avg time (s)",      f"avg time  (workers={workers_max})"),
        ("steal_pct",   "steal ratio (%)",   f"steal ratio  (workers={workers_max})"),
        ("minor_col",   "minor collections", f"minor GC cols  (workers={workers_max})"),
...
# ── Figure 3: resize ops — one subplot per (pool_mode x starting_size) ────────

def fig_resize_ops(idx, shrink_policies, pool_modes, sizes, size_ranks,
                   workers_list, extra_colors, out_path, title_prefix):
    nrows, ncols = len(sizes), len(pool_modes)
    fig, axes = plt.subplots(nrows, ncols,
                             figsize=(5*ncols, 3.8*nrows), squeeze=False)
    fig.suptitle(f"{title_prefix}  -  resize operations vs workers",
                 fontsize=12, fontweight="bold")

    for ri, size in enumerate(sizes):
        for ci, pool in enumerate(pool_modes):
            ax  = axes[ri, ci]
            ax2 = ax.twinx()
            ax.set_title(f"pool={pool}  start_sz={size}", fontsize=9)
            ax.set_xlabel("workers", fontsize=8)
            ax.set_ylabel("grows",   fontsize=8, color="#185FA5")
            ax2.set_ylabel("shrinks",fontsize=8, color="#D85A30")
            ax.set_xticks(workers_list)
            ax.tick_params(labelsize=8)
            ...
            for shrink in shrink_policies:
                color = _shrink_color(shrink, extra_colors)
                g = [get(idx, shrink, pool, size, w, "grows")   for w in workers_list]
                s = [get(idx, shrink, pool, size, w, "shrinks") for w in workers_list]
                g = [v if v is not None else np.nan for v in g]
                s = [v if v is not None else np.nan for v in s]
                if not all(np.isnan(v) for v in g):
                    ax.plot(workers_list, g, color=color,
                            linestyle="-", marker="o", linewidth=1.8,
                            label=f"grows/{shrink}")
                if not all(np.isnan(v) for v in s):
                    ax2.plot(workers_list, s, color=color,
                             linestyle="--", marker="s", linewidth=1.8,
                             label=f"shrinks/{shrink}")

            l1, n1 = ax.get_legend_handles_labels()
            l2, n2 = ax2.get_legend_handles_labels()
            ax.legend(l1+l2, n1+n2, fontsize=6.5, loc="upper left")

    fig.tight_layout()
    path = out_path.replace(".png", "_resize_ops.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"saved -> {path}")
...
def make_plots(rows, out_path, title_prefix, filter_size):
    ...
    fig_fixed_workers(idx, shrink_policies, pool_modes, sizes, size_ranks,
                      workers_max, extra_colors, out_path, title_prefix)
    fig_resize_ops(idx, shrink_policies, pool_modes, sizes, size_ranks,
                   workers_list, extra_colors, out_path, title_prefix)
